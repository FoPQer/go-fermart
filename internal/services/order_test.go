...
func TestOrderService_GetOrderByID(t *testing.T) {
	...
	t.Run("returns order by id", func(t *testing.T) {
		want := &models.Order{ID: "79927398713", Status: "PROCESSED"}
		repo := &stubOrderRepo{
			getOrderByIDFn: func(ctx context.Context, orderID string) (*models.Order, error) {
				if orderID != "79927398713" {
					t.Fatalf("unexpected orderID: %s", orderID)
		...
		got, err := svc.GetOrderByID(context.Background(), "79927398713")
		if err != nil {
			t.Fatalf("expected no error, got %v", err)
		}
		if got != want {
	...
	t.Run("wraps repository error", func(t *testing.T) {
		...
		_, err := svc.GetOrderByID(context.Background(), "79927398713")
		if err == nil {
			t.Fatal("expected error, got nil")
		}
		if !strings.Contains(err.Error(), "failed to get order") {
			t.Fatalf("expected wrapped error, got %v", err)
		}
	})
}
